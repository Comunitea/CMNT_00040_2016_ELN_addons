...
class process_document_queue_wizard(osv.osv_memory):
    _name = 'process.document.queue.wizard'
    _columns = {
    ...
    def default_get(self, cr, uid, fields, context=None):
        ...
        if context.get('active_model', False) and context['active_model'] == 'scan.document':
            ...
            ids = self.pool.get('scan.document').search(cr, uid, [('state','=','verified')], context=context)
    ...
    def act_process(self, cr, uid, ids, context):
        ...
        if context.get('active_model', False) and context['active_model'] == 'scan.document':
            ...
            ids_doc = self.pool.get('scan.document').search(cr, uid, [('state','=','verified')], context=context)
        form_obj = self.browse(cr, uid, ids)[0]
        if form_obj.background:
            signal = 'verified_to_processing'
        else:
            signal = 'process_document'
