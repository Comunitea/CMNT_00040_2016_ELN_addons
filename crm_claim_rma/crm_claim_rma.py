...
class substate_substate(orm.Model):
    ...
    _columns = {
        'name': fields.char('Sub state', size=128, required=True, translate=True),
        'substate_descr' : fields.text('Description', translate=True,
...
#        return res
    ...
    _columns = {
        ...
        'claim_id': fields.many2one('crm.claim', 'Related claim', required=True, ondelete='cascade',
        ...
        'move_out_id': fields.many2one('stock.move', 'Move Line from picking out',
                                       help='The move line related to the returned product'),
        'company_id': fields.related('claim_id', 'company_id', type='many2one', relation='res.company', string='Company', store=True, readonly=True),
    }
...
#        'out_supplier_picking_id': fields.many2one('stock.picking', 'Return From Supplier Picking', required=False, select=True),
    ...
    def onchange_partner_address_id(self, cr, uid, ids, add, email=False):
        ...
        if not res['value']['email_from'] or not res['value']['partner_phone']:
            ...
            if not address.partner_id.address:#TODO: Quizás poner en la vista el domain para solo direcciones del cliente. Pedro Gómez
                return res
