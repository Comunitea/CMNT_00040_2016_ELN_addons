...
export class SlideopPage {
    ...
    last_read: number = 0
    last_scan: ''
    WAITING: ['location_dest_id', 'product_id', 'lot_id', 'location_id', 'location_dest_id']
    waiting: number
    ...
    constructor(public navCtrl: NavController, private modalCtrl: ModalController, private toastCtrl: ToastController, public navParams: NavParams, private formBuilder: FormBuilder, private alertCtrl: AlertController, private odoo: OdooProvider) {
    ...
    get_op_ready() {
        ...
        this.op_ready = (!this.op['lot_id'] || (this.op['lot_id'] && this.op['lot_id']['checked']))
            && this.op['location_id']['checked']
            && this.op['location_dest_id']['checked']
            && (this.op['qty_done'] > 0)
        console.log("jaja si", this.op_ready)
    ...
    cargarOp(id = 0) {
        ...
        console.log(model, method, values)
    ...
    showSerial(product_id, qty) {
        ...
        var values = { 'product_ids': [product_id], 'qty': qty, 'op_id': this.op_id, 'lot_id': this.op['lot_id']['id'], 'location_id': this.op['location_id']['id'], 'move_id': this.op['move_id'] }
        console.log("antes de", 'stock.production.lot', method, values)
        this.odoo.execute('stock.production.lot', method, values).then((lot_ids) => {
            console.log(lot_ids, values)
    ...
    find_in_server(str) {
        var values = {
            'model': ['stock.production.lot', 'stock.location', 'product.product'],
            'search_str': str,
