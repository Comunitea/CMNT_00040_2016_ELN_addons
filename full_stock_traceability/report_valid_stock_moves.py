...
class valid_stock_moves(osv.osv):
    ...
    _columns = {
            ...
            'partner_id': fields.many2one('res.partner', 'Customer/Supplier', select=True, readonly=True),
    ...
    def init(self, cr):
        ...
        cr.execute("""
            create or replace view valid_stock_moves as (
                select stock_move.id as id, stock_move.name as name, stock_move.product_id as product_id, product_qty, product_uom, prodlot_id, stock_production_lot.use_date as expiry_date,
                    supplier, tracking_id, product_packaging, picking_id, location_id, location_dest_id, stock_move.date as date,
                    date_expected, state, production_id, partner_id
        ...
        #Se añade 'and location_id != location_dest_id' para que no salgan movimientos internos que tengan origen y destino en
        #la misma ubicación        

    def unlink(self, cr, uid, ids, context={}):
        """not can delete, because is database view"""
