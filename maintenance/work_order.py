...
class work_order(osv.osv):
    ...
    _columns = {
        ...
        'request_id': fields.many2one('intervention.request', 'Origin request', required=False),
        ...
        'fecha': fields.datetime('Request date'),
        'assigned_department_id': fields.many2one('hr.department', 'Assigned department', required=False),
        'origin_department_id': fields.many2one('hr.department', 'Origin department', required=False),
        'stock_moves_ids': fields.one2many('stock.move', 'work_order_id', 'Associated movement', required=False),
        'horas_ids': fields.one2many('work.order.time.report', 'work_order_id', 'Timesheet', required=False),
        'other_service_ids': fields.one2many('work.order.other.services', 'work_order_id', 'Other concepts', required=False),
        'purchase_ids': fields.one2many('purchase.order', 'work_order_id', 'Asociated purchases', required=False),
        ...
        'responsable_id': fields.many2one('res.users', 'Responsible', required=False),
        ...
        'grupo': fields.function(_get_grupo, method=True, type='boolean', string='Group', store=False),
        'deteccion': fields.text('Detection'),
        'sintoma': fields.text('Sign'),
        'efecto': fields.text('effect'),
        'planta': fields.function(_get_planta, method=True, type='char', string='Floor',
                      store={'work.order':(lambda self, cr, uid, ids, c={}: ids, ['elements_ids'], 10)}),
        'contrata': fields.function(_get_contrata, method=True, type='char', string='Contractual', store=False),
        'elements_list':fields.function(_get_element_list, method=True, type='char', string='Elements string', store=False),
        'total_other_service': fields.function(_get_total_other_service, method=True, type='float', string='Other concepts total', store=False),
        'total_servicios_internos': fields.function(_get_total_servicios, method=True, type='float', string='Total internal services', store=False),
        'total_servicios_externos': fields.function(_get_total_servicios, method=True, type='float', string='Total external services', store=False),
    }
    _defaults = {
        'state':'draft',
    ...
    def work_order_open(self, cr, uid, ids, context=None):
        ...
        for order in orders:
            ...
            self.pool.get('work.order').write(cr, uid, order.id, {'state': 'open', 'initial_date': initial_date}, context)
        return True
    ...
    def work_order_done(self, cr, uid, ids, context=None):
        ...
        for orden in ordenes:
            ...
            order_obj.write(cr, uid, orden.id, {'state': 'done', 'final_date': final_date}, context)
        return True
