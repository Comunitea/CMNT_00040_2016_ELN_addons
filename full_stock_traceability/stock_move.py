...
class stock_move(osv.osv):
    ...
    _columns = {
            ...
            'required_lots': fields.related('product_id','track_production', string='Required Lots', readonly=False,type='boolean'),
        }
    ...
    def get_moves_in_location(self, cr, uid, location_id, prodlot_id, qty = 0.0, id = False):
        ...
        domain = [('location_dest_id', '=', location_id), ('prodlot_id', '=', prodlot_id), ('location_id', '!=', location_id), ('state', '!=', 'cancel')]

        def get_all_child_ids(move, child_moves=[]):
            childs = [x for x in move.move_history_ids]

            if childs:
                for child in childs:
                    child_moves.append(child.id)
                    child_moves = list(set(child_moves)) #L1
                    child_moves.extend(get_all_child_ids(child, child_moves))
                    # Añadidas las dos lineas (L1 y L2):
                    # child_moves = list(set(child_moves))
                    # para eliminar elementos duplicados. En ciertos casos
                    # la lista crecía al cerrar órdenes de producción (aunque se usa en otros sitios también)
                    # de forma infinita hasta agotar la memoria y siempre con los mismos elementos duplicados
                    # Pedro Gómez (07/07/2015)
                    # con L2 está probado y funciona. L1 se añade el 13/10/2015 porque mientras se esté en recursividad también se duplica)
                    child_moves = list(set(child_moves)) #L2

            return child_moves

        if id:
            move = self.browse(cr, uid, id)
            domain.extend([('id','not in', get_all_child_ids(move))])

        ids = self.search(cr, uid, domain, order="id desc")
        if len(ids) == 1:
            moves = ids
        elif len(ids) == 0:
            return moves
            ...
            if qty:
                ...
                move_id = self.search(cr, uid, domain, order="id desc")
    ...
    def onchange_product_id(self, cr, uid, ids, prod_id=False, loc_id=False, loc_dest_id=False, address_id=False, prod_qty=0.0, prodlot_id=False, context=None):
        ...
        if address_id:
            partner_lang = partner_address_obj.browse(cr, uid, address_id, context=context).partner_id.lang
            if partner_lang:
                lang = partner_lang
        context_partner = dict(context, lang=lang)

        if not parent_result: #No hay product_id
            return parent_result

        #gets the object of the product
        obj_product_id = self.pool.get('product.product').browse(cr, uid, prod_id)
        product_name = self.pool.get('product.product').name_get(cr, uid, [prod_id], context=context_partner)[0][1]
        ...
        if result.get('value', False) and not result.get('warning', False) and parent_result.get('value') and parent_result['value'].get('name'):
            ...
            result['value']['product_uos'] = parent_result['value']['product_uos']
            result['value']['product_qty'] = parent_result['value']['product_qty']
            result['value']['product_uos_qty'] = parent_result['value']['product_uos_qty']
            return result
        if not result.get('value', False):
            result['value'] = {}

        result['value']['name'] = product_name
        result['value']['product_uom'] = obj_product_id.product_tmpl_id.uom_id.id
        result['value']['product_uos'] = obj_product_id.product_tmpl_id.uos_id.id
        result['value']['product_qty'] = parent_result['value']['product_qty']
        result['value']['product_uos_qty'] = parent_result['value']['product_uos_qty']

        if prod_id:
            if obj_product_id.track_production:
                result['value']['required_lots'] = True
            else:
                result['value']['required_lots'] = False
        else:
            result['value']['required_lots'] = False
    ...
    def onchange_lot_id(self, cr, uid, ids, prodlot_id=False, product_qty=False, loc_id=False, product_id=False, uom_id=False, context=None):
        """extends this event for warning when insufficient stock in lot"""
    ...
    def _create_lot(self, cr, uid, ids, product_id, prefix=False):
        """Overwrites this method to set product sequence to lot passing product in context"""
    ...
    def action_cancel(self, cr, uid, ids, context=None):
        ...
        if context is None:
            context = {}
        #Comento la siguiente linea porque no le encuentro sentido (Pedro Gómez). 
        #Al tenerla falla el proceso cancel_cascade
        #y además el campo move_dest_id ya se pone a falso en el método original action_cancel en addons/stock/stock.py
    ...
    def move_reserve_split(self, cr, uid, ids, context=None):
        ...
        for move in self.browse(cr, uid, ids, context=context):
            ...
            if tlots:
                ...
                proc_obj = False
                if procurement_ids:
                    self.pool.get('procurement.order').write(cr, uid, procurement_ids[0], {
                                                'product_qty': tlots[0][2] >= quantity and qty_towrite or self.pool.get('product.uom')._compute_qty(cr, uid, move.product_id.uom_id.id, tlots[0][2], move.product_uom.id)
                                                })
                    ...
                    for record in tlots:
                        ...
                        if move.move_dest_id:
                            ...
                            move_dest_id = self.create(cr, uid, {
                                ...
                                'product_uos_qty': False, #qty_towrite,
                                'product_uos': move.product_uos and move.product_uos.id or False, #move.product_uom.id,
                        ...
                        if proc_obj and not move.product_id.not_do_procurement:
                            proc_id = self.pool.get('procurement.order').create(cr, uid, {
                                ...
                                'product_uom': move.product_uom.id,
                                'product_uos_qty': False, #qty_towrite,
                                'product_uos': proc_obj.product_uos and proc_obj.product_uos.id or False, #move.product_uom.id,
                                'location_id': record[1],
